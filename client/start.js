...
const initApp = () => {
    const $startPrivateSale = document.getElementById('startPrivateSale');
    const $privateSaleResult = document.getElementById('privateSaleResult');

    $startPrivateSale.addEventListener('submit', (e) => {
        e.preventDefault();
        const duration = e.target.elements[0].value;
        const price = e.target.elements[1].value;
        const availableTokens = e.target.elements[2].value;
        const minPurchase = e.target.elements[3].value;
        const maxPurchase = e.target.elements[4].value;
        privateSale.methods.start(duration, price, availableTokens, minPurchase, maxPurchase).send({from: accounts[0]}).then(result => {
            $privateSaleResult.innerHTML = `Private Sale Started successfully`;
            document.getElementById("startPrivateSale").style.display="none";
        }).catch(_e => {
            console.log(e);
            $privateSaleResult.innerHTML = `ERROR starting private sale`;
        })
    });
